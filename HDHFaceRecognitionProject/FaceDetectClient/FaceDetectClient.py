...
class WindowClass(QMainWindow, form_class) :
    ...
    def recvImage(self, img):      
        self.QPIXMAX_IMAGE.setPixmap(QPixmap.fromImage(img))
 
    def closeEvent(self, e):
        self.videocontroller.stopCam()  
    
    
    def CamStart(self):
        if(self.IsPlay is False):
            self.videocontroller.CommandWork(Command.CamStart)
            self.IsPlay = True
            
    def CamStop(self):
        if(self.IsPlay is True):
            self.videocontroller.CommandWork(Command.CamStop)
            self.IsPlay = False
    ...
    def Makecsvfile(self) :
        if self.IsPlay is False:
            ...
            for Image in file_list:
                LearnImage_Path = fname + '/' + Image
                wr.writerow([LearnImage_Path, 'Handonghee', 3])
    ...
    def FaceThresholdChangedFuncion(self):               
        ...
        if self.IsPlay is False:
            ...
            self.videocontroller.conf_threshold = self.FACEDETECTHSLIDER_FACETHREAHOLD.value() / 100
